...
document.getElementById("getPokemon").addEventListener("keyup", function (event) {
  if (event.keyCode === 13) {
...
// Returns the requisition of the API and allows data to be used in another functions 
...
let pokeImage = (receiveData) => {
  ...
  getPokemonData(receiveData)
    ...
    .then(pokeData => {
      ...
      document.getElementById('firstSlide').style.
...
let pokeName = (receiveData) => {
  ...
  getPokemonData(receiveData)
    .then(pokeData => {
      ...
      document.querySelector('.pokeName').innerText = `${pokeData.name.charAt(0).toUpperCase() + pokeData.name.slice(1)}`;
    })
};
...
let pokeAbilities = (receiveData, regex) => {
  ...
  getPokemonData(receiveData)
    .then(pokeData => {
          ...
          .then(nameOfTheAbility => {
            ...
            nameOfTheAbility.data.effect_entries.forEach(effectDescription => {
              ...
              if (effectDescription.language.name == "en") {
                ...
                // The pokemon ability is received, with first letter capitalized and the effect's descriptions
                ability.textContent =
                  `${pokeAbilities.ability.name.charAt(0).toUpperCase() 
                    + pokeAbilities.ability.name.slice(1)} effect: ${abilitiesEffects}`;
              }
            })
          })
      })
      ...
      let pokeAbilitiesEdited = capitalizeNames(abilitiesNames).join().replace(regex, ' | ');
      ...
      pokeData.types.forEach(pokeTypes => {
        ...
        axios.get(`https://pokeapi.co/api/v2/type/${pokeTypes.type.name}/`)
          .then(nameOfTheType => {
            ...
            let toIntersections = halfDamageTo.filter(types => doubleDamageTo.includes(types));
            ...
            let fromIntersections = halfDamageFrom.filter(types => doubleDamageFrom.includes(types));
            ...
            let noDamageHalfFromIntersections = halfDamageFrom.filter(types => noDamageFrom.includes(types));
            for (let i = 0; i < noDamageHalfFromIntersections.length; i++) {
              if (!noDamageHalfFromIntersections == '') {
                let typeIndex = halfDamageFrom.indexOf(noDamageHalfFromIntersections[i]);
                halfDamageFrom.splice(typeIndex, 1);
                noDamageHalfFromIntersections.splice(i, 1);
                i--
              }
            }
            let noDamageHalfToIntersections = halfDamageTo.filter(types => noDamageTo.includes(types));
            for (let i = 0; i < noDamageHalfToIntersections.length; i++) {
              if (!noDamageHalfToIntersections == '') {
                let typeIndex = halfDamageTo.indexOf(noDamageHalfToIntersections[i]);
                halfDamageTo.splice(typeIndex, 1);
                noDamageHalfToIntersections.splice(i, 1);
                i--
              }
            }
            let noDamageDoubleFromIntersections = doubleDamageFrom.filter(types => noDamageFrom.includes(types));
            for (let i = 0; i < noDamageDoubleFromIntersections.length; i++) {
              if (!noDamageDoubleFromIntersections == '') {
                let typeIndex = doubleDamageFrom.indexOf(noDamageDoubleFromIntersections[i]);
                doubleDamageFrom.splice(typeIndex, 1);
                noDamageDoubleFromIntersections.splice(i, 1);
                i--
              }
            }
            let noDamageDoubleToIntersections = doubleDamageTo.filter(types => noDamageTo.includes(types));
            for (let i = 0; i < noDamageDoubleToIntersections.length; i++) {
              if (!noDamageDoubleToIntersections == '') {
                let typeIndex = doubleDamageTo.indexOf(noDamageDoubleToIntersections[i]);
